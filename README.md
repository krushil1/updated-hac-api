...
``` javascript
axios.get("https://updated-hac-api.vercel.app/students/schedule?username=john&password=doe").then((res) => {
...
#### Get student's class information (Name & Grade) with assignments for the current quarter
...
Example Request (Axios):
``` javascript
axios.get("https://updated-hac-api.vercel.app/students/currentclasses?username=john&password=doe").then((res) => {
    console.log(res.data);
}).catch((error) => {
    console.log(error);
})
